...
const ChatComponent = () => {
  ...
  const [conversationTitles, setConversationTitles] = useState([]); // New state for conversation titles
  ...
  const handleNewChat = () => {
    // Clear the conversation and reset the conversation ID
    setMessages([]);
    setInput('');
    setConversationId('');

    // Fetch conversation titles to update the sidebar
    ...
    if (input.trim() !== '') {
      ...
      axios.post("http://localhost:4000/gab/conversation", { input: input, conversationId: conversationId })
        .then(response => {
          ...
          // If there's no conversation ID yet, set it to the received one from the server
          if (!conversationId) {
  ...
  useEffect(() => {
    // Fetch conversation titles when component mounts
    fetchConversationTitles();
